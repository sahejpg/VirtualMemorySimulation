...
pfn_t free_frame(void) {
    ...
    if (frame_table[victim_pfn].mapped) {
        vpn_t curr_vpn = frame_table[victim_pfn].vpn;
        pte_t *phys_victim = ((pte_t *)(mem + frame_table[victim_pfn].process->saved_ptbr * PAGE_SIZE)) + curr_vpn;
        pte_t *victim_frame = (pte_t *)(mem + victim_pfn * PAGE_SIZE);

        if (phys_victim->dirty == 1) {
            swap_write(phys_victim, (void*)victim_frame); // cast to void?
            phys_victim->dirty = 0;
